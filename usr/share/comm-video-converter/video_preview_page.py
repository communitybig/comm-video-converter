...
class VideoPreviewPage:
    ...
    def _create_page(self):
        ...
        paned.set_shrink_start_child(False)  # Don't shrink the top part below its minimum
        paned.set_shrink_end_child(False)    # Don't shrink the bottom part below its minimum
        ...
        self.position_scale = Gtk.Scale(orientation=Gtk.Orientation.HORIZONTAL, adjustment=adjustment)
        self.position_scale.set_draw_value(False)
        self.position_scale.set_hexpand(True)
        self.position_changed_handler_id = self.position_scale.connect("value-changed", self.on_position_changed)
        ...
        result_label.set_markup("<small>" + _("Final size: calculating...") + "</small>")
        ...
        self.brightness_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, -1.0, 1.0, 0.05)
        self.brightness_scale.set_value(self.brightness)
        self.brightness_scale.set_size_request(400, -1)  # Double the width from 200 to 400
        ...
        self.contrast_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0.0, 2.0, 0.05)
        self.contrast_scale.set_value(self.contrast)
        self.contrast_scale.set_size_request(400, -1)  # Double the width from 200 to 400
        ...
        self.saturation_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0.0, 2.0, 0.05)
        self.saturation_scale.set_value(self.saturation)
        self.saturation_scale.set_size_request(400, -1)  # Double the width from 200 to 400
        ...
        self.exposure_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, -3.0, 3.0, 0.1)
        self.exposure_scale.set_value(self.exposure)
        self.exposure_scale.set_size_request(400, -1)  # Double the width from 200 to 400
        ...
        self.gamma_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0.0, 16.0, 0.1)
        ...
        self.gamma_r_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0.0, 16.0, 0.1)
        ...
        self.gamma_g_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0.0, 16.0, 0.1)
        ...
        self.gamma_b_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0.0, 16.0, 0.1)
        ...
        self.gamma_weight_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, 0.0, 1.0, 0.01)
        self.gamma_weight_scale.set_value(self.gamma_weight)
        self.gamma_weight_scale.set_size_request(400, -1)  # Double the width from 200 to 400
        ...
        self.hue_scale = Gtk.Scale.new_with_range(Gtk.Orientation.HORIZONTAL, -3.14, 3.14, 0.05)
    ...
    def find_slider_gizmo(self, slider):
        ...
        def find_gizmo(widget):
            ...
            # Check next sibling
            sibling = widget.get_next_sibling() if hasattr(widget, 'get_next_sibling') else None
    ...
    def load_video(self, file_path):
        ...
        # Update video info labels with loading message
        self.info_filename_label.set_text(_("Loading..."))
        self.info_dimensions_label.set_text(_("Loading..."))
        self.info_codec_label.set_text(_("Loading..."))
        self.info_format_label.set_text(_("Loading..."))  # Initialize format label
        self.info_filesize_label.set_text(_("Loading..."))
        self.info_duration_label.set_text(_("Loading..."))
        self.info_fps_label.set_text(_("Loading..."))
        ...
        try:
            ...
            milliseconds = int((self.video_duration % 1) * 1000)
            ...
            self.info_dimensions_label.set_text(f"{self.video_width}×{self.video_height}")
    ...
    def extract_frame(self, position):
        ...
        try:
            ...
            if position >= safe_end:
                print(f"Position {position} exceeds safe video duration {safe_end}, limiting to safe end")
                position = safe_end
                ...
                if hasattr(self, 'position_scale') and hasattr(self, 'position_changed_handler_id'):
                    self.position_scale.handler_block(self.position_changed_handler_id)
                    self.position_scale.set_value(position)
                    self.position_scale.handler_unblock(self.position_changed_handler_id)
            
            # Generate a temp filename for the frame
            output_file = os.path.join(self.temp_preview_dir, f"frame_{int(position*100)}.jpg")
            ...
            if self.crop_left > 0 or self.crop_right > 0 or self.crop_top > 0 or self.crop_bottom > 0:
                ...
                filters.append(f"crop={crop_width}:{crop_height}:{self.crop_left}:{self.crop_top}")
    ...
    def on_save_frame_response(self, dialog, result):
        ...
        try:
            ...
            if file:
                ...
                if self.crop_enabled_switch.get_active():
                    filters.append(f"crop={self.crop_width}:{self.crop_height}:{self.crop_x}:{self.crop_y}")
                ...
                # High quality export
                cmd = [
                    "ffmpeg", "-y",  # Don't include -v quiet to see errors
                ...
                if result.returncode == 0 and os.path.exists(file_path):
                    ...
                    self.app.show_error_dialog(_("Error saving frame: {0}").format(error_msg))
                
        except GLib.Error as error:
            print(f"Error saving frame: {error.message}")
            self.app.show_error_dialog(_("Error saving frame: {0}").format(error.message))
    ...
    def cleanup(self):
        ...
        try:
            if hasattr(self, 'temp_preview_dir') and os.path.exists(self.temp_preview_dir):
    ...
    def set_crop_visibility(self, visible=True):
        ...
        for child in self.page.get_first_child().get_first_child().get_first_child().get_child().get_children():
            if isinstance(child, Adw.PreferencesGroup):
                if child.get_title() == _("Crop Video"):
    ...
    def update_frame_counter(self, position):
        ...
        current_frame = int(position * fps) + 1  # +1 because frames typically count from 1
